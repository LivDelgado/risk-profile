...
def test_apply_deduct_from_risk_score():
    rule = DeductFromRiskScore(user, base_score)
...
def test_younger_than_30_should_apply():
    user.age = 20
    rule = DeductWhenYoungerThan30(user, base_score)
...
def test_age_between_30_and_40_should_apply_30():
    user.age = 30
    rule = DeductWhenBetween30and40(user, base_score)
    should_apply = rule.should_apply()
    assert should_apply is True
...
def test_income_over_200k_should_apply():
    user.income = 201
    rule = DeductWhenIncomeOver200k(user, base_score)
    should_apply = rule.should_apply()
    assert should_apply is True
...
def test_deduct_when_married_should_apply():
    ...
    rule = DeductWhenIsMarried(user, base_score)
    should_apply = rule.should_apply()
    assert should_apply is True
